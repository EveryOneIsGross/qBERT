...
class ParallelBERTGenerator(nn.Module):
    ...
    def optimize_sequence_mcts(self, initial_text: str, num_tokens: int, diffusion_steps: int = None) -> str:
        """
        Optimize sequence using MCTS with a simplified approach.
        
        This method:
        1. Runs multiple diffusion processes to gather candidates
        2. Uses beam search to find optimal combinations
        3. Applies a single coherence-based refinement pass
        """
        if diffusion_steps is None:
            diffusion_steps = self.config.diffusion_steps
        
        # Step 1: Run diffusion generation multiple times, tracking candidates
        ...
        # Step 2: For each position, get top candidates
        best_candidates = {}
        for pos, candidates in token_candidates.items():
            # Sort by coherence score and keep top 3 unique candidates
        ...
        # Step 4: Select best sequence based on global coherence
        if sequences:
            best_sequence = self._select_best_sequence(sequences, initial_text)
        else:
    ...
    def _select_best_sequence(self, sequences, initial_text):
        """Select the best sequence based on coherence and factual accuracy."""
        ...
        for sequence, _ in sequences[:3]:  # Consider top 3 sequences
            ...
            coherence_score = self._calculate_simple_coherence(sequence)
            
            # For QA contexts, check factual accuracy
            if is_qa:
                full_text = self.tokenizer.decode(sequence)
                qa_parts = full_text.split("ASSISTANT:")
                if len(qa_parts) > 1:
                    question = qa_parts[0].replace("USER:", "").strip()
                    answer = qa_parts[1].strip()
                    
                    # Simple factual bonus for common questions
                    if "capital of France" in question.lower() and "paris" in answer.lower():
                        coherence_score += 1.0
            
            if coherence_score > best_global_score:
                best_global_score = coherence_score
                best_sequence = sequence
        
        return best_sequence

    def _calculate_simple_coherence(self, sequence):
        """Calculate coherence using a simplified approach."""
        with torch.no_grad():
            # Get embeddings
            ...
            # Calculate pairwise similarity in chunks
            ...
            for i in range(0, len(sequence) - chunk_size + 1, chunk_size // 2):
                ...
                sim_matrix = torch.mm(norm_embs, norm_embs.transpose(0, 1))
                
                # Exclude self-similarity
                mask = 1.0 - torch.eye(sim_matrix.size(0), device=self.config.device)
                coherence_score += (sim_matrix * mask).sum() / (mask.sum() + 1e-9)
                total_chunks += 1
